    ...
    <div className="py-4 relative px-8 mb-24 grid place-content-center   mx-auto max-w-7xl w-11/12 backdrop-blur-[2px]">
      ...
      <form className="flex gap-3 w-full max-sm:grid">
        <div className="relative flex-1 my-5 max-sm:mb-0">
          <input type="search" value={searchTerm} onChange={handleChange} autoComplete="off" placeholder="Search by company name, business category, brand or keyword ..." name="search" id="search" className="px-4 py-2 pr-8 w-full text-gray-900 overflow-ellipsis bg-gray-200 rounded-md outline-none" />
        ...
        <div className="my-5 max-sm:mb-0 max-sm:mt-2 max-sm:hidden relative flex-[0.3] max-lg:hover:flex-1 ease-in-out duration-500">
          <input type="search" autoComplete="off" placeholder="In which tole or ward?" name="search" id="search" className="px-4 py-2 w-full text-gray-900 overflow-ellipsis bg-gray-200 rounded-md outline-none" />
        </div>
        <div className="flex items-center max-sm:justify-center">
          <button onClick={handleSubmit} className="px-4 py-1 h-9 text-white bg-green-500 rounded-sm border-2 outline-none select-none font-font-1 border-l-teal-100">search</button>
        </div>
      </form>
      {message &&
