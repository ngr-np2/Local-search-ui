  ...
  return (
    <>
      {datas.map((data, idx) => (
        <div
          key={idx}
          className="relative max-w-xs text-center rounded-lg shadow-md cursor-pointer bg-stone-100 group hover:shadow-lg"
        >
          <div className="overflow-hidden h-56">
            {imageLoaded ||
              (err && (
                <img
                  src="/ITAHARI.jpeg"
                  className="object-cover mx-auto w-64 max-w-sm h-56 bg-bottom bg-cover transition-all duration-500 ease-in-out max-auto group-hover:scale-105"
                  alt="Placeholder"
                />
              ))}
          ...
          <button
            title={`View Alll ${data.total} Business in this ${data.name} Category`}
